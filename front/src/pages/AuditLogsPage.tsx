...
import { Input, Select, Tag, DatePicker, Button, Popconfirm, message } from 'antd';
    ...
    <div>
      <div className="flex items-center justify-between mb-3 gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Popconfirm title="Delete logs before keepDays?" description="根据保留天数清理更早的日志" onConfirm={async () => { await retentionDelete(30); message.success('Retention applied'); setReloadTick(t=>t+1); }}>
            <Button>Retention 30d</Button>
          </Popconfirm>
          <Popconfirm title="Purge logs before date?" onConfirm={async () => { if (!from) { message.warning('请选择起始时间作为 before'); return; } await purgeAuditLogs(from); message.success('Purged'); setReloadTick(t=>t+1); }}>
            <Button danger>Purge Before From</Button>
          </Popconfirm>
          {selectedIds.length > 0 && (
            <div className="flex items-center gap-2">
              <Popconfirm title={`Bulk delete ${selectedIds.length} logs?`} onConfirm={async () => { await bulkDeleteAuditLogs(selectedIds); message.success('Bulk deleted'); setReloadTick(t=>t+1); setSelectedIds([]); }}>
                <Button danger>Bulk Delete Selected</Button>
              </Popconfirm>
              <Popconfirm title={`Purge ${selectedIds.length} logs?`} onConfirm={async () => { await purgeByIdsAuditLogs(selectedIds); message.success('Purged by ids'); setReloadTick(t=>t+1); setSelectedIds([]); }}>
